...
def draw_chord_on_note(canvas_obj, x, y, note_radius, chord_number):
    x_offset = note_radius + (CHORD_NOTE_OFFSET_MM * mm)
    canvas_obj.setFont("Helvetica-Bold", CHORD_FONT_SIZE)
    canvas_obj.saveState()
    canvas_obj.translate(x + x_offset, y)
    canvas_obj.rotate(90)
    canvas_obj.drawString(0, -(CHORD_FONT_SIZE * 0.3), str(chord_number))
    canvas_obj.restoreState()


def draw_chords_between_notes(canvas_obj, x1, y1, x2, y2, chord_numbers):
    if not chord_numbers:
        return
    mx = (x1 + x2) / 2
    my = (y1 + y2) / 2
    count = len(chord_numbers)
    spacing = CHORD_BETWEEN_SPACING_MM * mm
    start_x = mx - ((count - 1) * spacing / 2)
    canvas_obj.setFont("Helvetica-Bold", CHORD_FONT_SIZE)
    for i, chord_number in enumerate(chord_numbers):
        x = start_x + i * spacing
        chord_text = str(chord_number)
...
def parse_melody_entry(entry):
    ...
    if len(entry) == 3:
        ...
        return {
            ...
            "duration": duration,
            "dotted": False,
            "chord_with_note": chord_number,
        }
    if len(entry) == 4:
        note_name, duration, dotted, chord_with_note = entry
        if chord_with_note is not None:
            chord_with_note = int(chord_with_note)
            if chord_with_note not in {1, 2, 3, 4, 5, 6}:
                raise ValueError(f"Invalid chord number: {chord_with_note}")
        return {
            "kind": "note",
            "note_name": note_name,
            "duration": duration,
...
def build_drawable_notes(melody_entries):
    ...
    for entry in parsed_melody:
        ...
        if entry["kind"] == "chord_between":
            pending_between_chords.append(entry["chord_number"])
            continue
...
def render_pdf(voice_melodies, piece_name="", output_pdf=OUTPUT_PDF_FILE):
    ...
    for voice_index, voice_id in enumerate(sorted(drawable_notes_by_voice.keys(), key=_voice_sort_key)):
        ...
        for x_position, (note_name, duration, dotted, rest_before, between_chords, chord_with_note) in zip(
            ...
            if chord_with_note is not None:
                draw_chord_on_note(c, x_position, y, note_radius, chord_with_note)
...
def serialize_project_data(voice_melodies, piece_name):
    return {
...
def save_project_data(voice_melodies, piece_name, output_path=INPUT_MELODY_FILE):
    data = serialize_project_data(voice_melodies, piece_name)
...
def run_gui():
    voice_melodies, initial_piece_name = load_project_data()
    ...
    dotted_check.grid(row=1, column=5, padx=(0, 10), sticky="w")
    ...
    def add_event():
        ...
        if event_type == "rest":
            ...
            if between_chord_text in {"1", "2", "3", "4", "5", "6"}:
                ...
                if has_prior_note:
                    voice_melodies[voice_id].append(("chord", int(between_chord_text)))
                    between_chord_var.set("none")

            chord_with_note = None
            if chord_var.get().strip() in {"1", "2", "3", "4", "5", "6"}:
                chord_with_note = int(chord_var.get().strip())
            if dotted_var.get():
    ...
    def add_between_chord():
        ...
        if chord_text not in {"1", "2", "3", "4", "5", "6"}:
            ...
            return
        voice_melodies[voice_id].append(("chord", int(chord_text)))
        refresh_event_list()
    ...
    def write_and_generate_pdf():
        try:
            ...
            save_project_data(voice_melodies, piece_name, INPUT_MELODY_FILE)
            output_pdf = output_pdf_var.get().strip() or OUTPUT_PDF_FILE
            if not output_pdf.lower().endswith(".pdf"):
                output_pdf += ".pdf"
                output_pdf_var.set(output_pdf)
            render_pdf(voice_melodies, piece_name, output_pdf)
            messagebox.showinfo(
                "Success",
